...
fn render_chip_registers(chip: &SimChip, area: Rect, buf: &mut Buffer) {
    ...
    let [labels_area, values_area] =
        Layout::horizontal([Constraint::Length(5), Constraint::Fill(1)]).areas(inner_area);

    let text = Text::from_iter(LABELS);
    Paragraph::new(text).render(labels_area, buf);

    let values_str = chip.registers().iter().map(|v| v.to_string());
    let mut text = Text::from(chip.stack_pointer().to_string());
    text += Line::from(chip.return_address().to_string());
    text.extend(values_str);
    Paragraph::new(text).render(values_area, buf);
...
fn render_chip_stack(chip: &SimChip, area: Rect, buf: &mut Buffer) {
    ...
    let values_str = chip.stack().iter().map(|v| v.to_string());
